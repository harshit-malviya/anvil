...
- Resolved PDF Merge output bloat and silent content loss (`docs/V2/PDF Tool Bug/TASK_pdf_merge_resource_dedup.md`):
  - Investigated Syncfusion `createTemplate()` object cloning behavior via direct path imports (`package:syncfusion_flutter_pdf/src/pdf/...`).
  - **Discovered Instance Aliasing Root Cause:** `createTemplate()` caches cloned objects in `_clonedObject` on the `PdfStream` instance. When multiple pages share a source resource dictionary, `createTemplate()` re-uses the `identical()` same `PdfStream` Dart instance in memory across page template resource dictionaries. Setting `dup.stream.isSkip = true` on what was assumed to be a duplicate stream copy mutated `canonical.isSkip` to `true`, causing Syncfusion's `_saveObjects()` to suppress canonical streams from saving and rendering pages blank.
  - **Implemented Hard Identity Guard:** In `_deduplicateResources` (`lib/core/services/pdf_isolate_worker.dart`), added an unconditional check `if (identical(canonical, dup.stream)) continue;` before every repointing and `isSkip` mutation. Shared aliased instances are preserved without mutation, while distinct duplicate stream instances are deduplicated safely.
  - Re-enabled `_deduplicateResources(destinationDoc)` on the live merge path.
  - Added 3 comprehensive regression/integration tests in `test/tools/pdf_merge/resource_dedup_test.dart` asserting per-page content integrity (non-blank page resources) and size bounds on catalog-level shared files (`संविधान सभा #1–3`, 29 pages intact), distinct per-page resource files (`05 - Sources of Indian Constitution`, 19 pages intact), and mega-bloat files (`merged_1786120641086.pdf`, 174 pages intact, size reduced from 42.80MB to 10.38MB, saving 32.42MB).
  - All 27 tests in `test/tools/pdf_merge/` passing cleanly.
...
- Implemented App Log / Debug Log Feature (`docs/App Log Feature/FEATURE_debug_log.md`):
  ...
  - Registered `/debug-log` route in `lib/core/router.dart` and wired hidden long press trigger on "OFFLINE WORKSHOP" in `lib/home/home_screen.dart`.
...
- Decision: Restracted deduplication strictly to `Subtype: Image` (`/Image`) XObjects (`_isImageStream`), explicitly excluding `Subtype: Form` (`/Form`) XObjects to prevent Form XObject template stream collapse.
- Decision: Computed stream hash keys using a composite key: SHA-256 of `dataStream` bytes + `Width`, `Height`, `BitsPerComponent`, `ColorSpace`, and `SMask` presence & hash to prevent false-positive merging of image streams with identical pixel content but differing transparency/color properties.
- Decision: Added an unconditional hard identity guard `if (identical(canonical, dup.stream)) continue;` in `_deduplicateResources` (`lib/core/services/pdf_isolate_worker.dart`). Because `createTemplate()` caches cloned objects on source instances (`_clonedObject`), multiple page templates sharing a source resource dictionary reference the `identical()` same `PdfStream` Dart object in memory. Checking `!identical(canonical, dup.stream)` before repointing or setting `isSkip = true` prevents canonical stream deletion while safely deduplicating distinct stream instances across separate files.
...
- **[FIXED] PDF Merge output bloat from duplicated shared resources:** `createTemplate()` snapshots resources per page, multiplying shared images/backgrounds across merged pages (~12x file size bloat). Resolved by walking Syncfusion internal object graph in memory (`_deduplicateResources` in `lib/core/services/pdf_isolate_worker.dart`), computing composite SHA-256 stream hashes, and re-pointing duplicate stream entries to a canonical reference. Resolved critical instance-aliasing bug where `createTemplate()` caches cloned objects in `_clonedObject`, causing multiple page templates to share `identical()` `PdfStream` Dart object instances in memory; added an unconditional hard identity guard `if (identical(canonical, dup.stream)) continue;` before every repointing and `isSkip` mutation to prevent canonical stream deletion and blank pages. Verified via production log entries `log_1786161071854004_0001` (failure repro), `log_1786164727587532_0001` (spot-check fix), and `log_1786164799436190_0002` (5-file mega-merge fix), along with 27 passing tests in `test/tools/pdf_merge/resource_dedup_test.dart`.
